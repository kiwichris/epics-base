...
initHandler::initHandler(
    ...
    const epicsRtemsInitHandler& handler_)
    : domain(domain_), name(name_), order(order_), handler(handler_) {
}
...
epicsRtemsInitRegister::epicsRtemsInitRegister(
    const char* domain, const char* name, const size_t order,
    const epicsRtemsInitHandler& handler) {
    epicsRtemsInitRegisterHandler(domain, name, order, handler);
}
...
int epicsRtemsInitRegisterHandler(
    ...
    handlers->emplace_back(domain, name, order, handler);
    return 0;
}
    ...
    std::for_each(
        ...
        [&step, &first_error](auto& handler) {
            ...
            std::cout << "] Init (" << handler.order
                      << ',' << step << '/' << handlers->size() << "): "
                      << handler.str() << std::endl << std::flush;
            int error;
            try {
