...
static void osdNTP_InitOnce(void *not_used)
{
    ...
    memset(&osdNTPPvt.rl, 0, sizeof(osdNTPPvt.rl));
...
static const iocshArg ntpWaitForSyncArg0 = { "host", iocshArgInt };
...
static void ntpInitHook(initHookState state)
{
    switch (state) {
...
{
    return initHookRegister(ntpInitHook);
}
